...
index_url_template = 'http://www.profightdb.com/cards/wwe-cards-pg{}-no-2.html?order=&type='  # goes from 1 to whatever
...
def parse(lastdate, lastpage):
    ...
    try:
        for index_counter in range(1,lastpage).__reversed__():
            browser.get(index_url_template.format(index_counter))
...
def test_model(model, event_dict):
    ...
    for match in event_dict['matches']:
        ...
        for wrestler in range(len(event_dict["teams"][0])):
            print("{}: {:.2%}".format(event_dict["teams"][0][wrestler], match_result[wrestler]/norm))
...
def last_page():
    browser = webdriver.Chrome("chromedriver.exe", chrome_options=chrome_options)
    browser.get(index_url_template.format(1))
