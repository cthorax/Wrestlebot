...
def make_prediction_series(alias, date, db, number_of_history_matches=10):
    from math import log10, ceil
    id = lookup(alias, db)
    
    index_list = ['wrestler_id', 'dob', 'nationality']
    index_template_list = ['days_since_match', 'won_match', 'wintype_match', 'title_match', 'matchtype_match', 'opponents_match', 
                 'allies_match']
    ...
    history_query = db.query("SELECT date, wintype, title, matchtype, competitors WHERE date < {date} AND competitors IN (\nSELECT id FROM competitors_table WHERE  ) FROM match_table ORDER BY date DESC".format()).as_dict()
